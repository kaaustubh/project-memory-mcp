...
- 2026-07-22 (v1.7.0): `install` now registers **every GitHub Copilot surface**, not just VS Code — user asked "can we not provision this to make it work with Copilot too?" then, after seeing it worked, "i need to cover all the copilot scenarios — cli, jetbrain's plugin etc" (their Cariad team uses Copilot, not Claude Code/Cursor). Added: **GitHub Copilot CLI** (`~/.copilot/mcp-config.json`, override via `$COPILOT_HOME`; `mcpServers` key like Claude/Cursor but each entry additionally needs `"type": "local"`), **JetBrains Copilot plugin** (`~/.config/github-copilot/intellij/mcp.json` mac/linux, `%APPDATA%\github-copilot\intellij\mcp.json` windows; same `servers`/`type:"stdio"` schema as VS Code), and **Visual Studio on Windows** (global `%USERPROFILE%\.mcp.json`, same `servers` schema, gated on `process.platform === "win32"` since it's a Windows-only IDE). Refactored the by-now-5x-repeated merge-into-JSON logic into one `registerMcp(cfgPath, topKey, entry, label)` helper — each call is independently try/caught so one client being absent/unwritable never blocks the others. Verified every schema/path against each client's current docs before writing code (none of this was in training-data knowledge — Copilot CLI and JetBrains support are 2025-era additions) and re-ran the throwaway-HOME install test to confirm all four new/changed config files come out with the right shape and still merge without clobbering pre-existing entries. Deliberately did NOT add Eclipse/Xcode (also Copilot-MCP-capable per GitHub's changelog) — user's ask was specifically "cli, jetbrains, etc" and those two have far less certain config-path documentation; add them only if asked.
