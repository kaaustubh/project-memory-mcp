...
- 2026-07-03 (branch `team-sync-validation`, UNRELEASED): Building the CHEAPEST rig to test the monetization hypothesis "will a team pay for SHARED memory?" BEFORE any backend — git IS the sync backend (a shared private repo = free auth/storage/history/hosting). New OPT-IN `sync` subcommand mirrors the memory files into `~/.project-memory-sync`, pushes/pulls against `PROJECT_MEMORY_SYNC_REMOTE`, and writes the merged result back to the LIVE files so the existing recall/search surface teammates' memory with ZERO other change (the value falls out of the current architecture — that's the point). Merge = git's BUILT-IN `union` driver (memory is append-heavy → grow-only set, the friendliest merge case) + a dedup pass, because union's one flaw is keeping BOTH copies of a line changed on both sides; dedup collapses issues by id (last-write-wins, so `resolve_issue` mutations win) and drops exact-duplicate `- `/`|` lines. Chose the mirror-copy approach (copy live→mirror, merge, copy back) over making ROOT a git repo because ROOT (~/code) is a tree of NESTED per-project repos. Prerequisite fix shipped in the same branch: `log_issue` ids are now `${project}-${randomBytes(3).hex}` not line-count-based — the old scheme collides across machines and was the one hard blocker for multi-writer merge (was already flagged in Known sharp edges). Deliberately NOT built: accounts, backend, dashboard, billing — those are only earned if the git rig produces real cross-author recall hits. Verified end-to-end with two fake machines + a bare remote: convergence, resolve-mutation LWW, and concurrent bullet union+dedup all pass.
