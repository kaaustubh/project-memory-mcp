...
- 2026-07-22 (v1.7.0): `install` now also registers **VS Code / GitHub Copilot** (user-profile `mcp.json`, `servers`/`type:"stdio"` schema), alongside Claude Code and Cursor — user asked "can we not provision this to make it work with Copilot too?" after a Slack-draft reframe surfaced their Cariad team uses Copilot, not Claude Code/Cursor. **Published to npm as 1.7.0 before the follow-up ask below landed** — so that release contains VS Code support only, not the CLI/JetBrains/Visual Studio work.
- 2026-07-22 (v1.8.0): Immediately after 1.7.0 published, user asked to "cover all the copilot scenarios — cli, jetbrain's plugin etc". Added **GitHub Copilot CLI** (`~/.copilot/mcp-config.json`, override via `$COPILOT_HOME`; `mcpServers` key like Claude/Cursor but each entry additionally needs `"type": "local"`), **JetBrains Copilot plugin** (`~/.config/github-copilot/intellij/mcp.json` mac/linux, `%APPDATA%\github-copilot\intellij\mcp.json` windows; same `servers`/`type:"stdio"` schema as VS Code), and **Visual Studio on Windows** (global `%USERPROFILE%\.mcp.json`, same `servers` schema, gated on `process.platform === "win32"`). Refactored the by-then-5x-repeated merge-into-JSON logic into one `registerMcp(cfgPath, topKey, entry, label)` helper — each call independently try/caught so one absent/unwritable client never blocks the others. Verified every schema/path against each client's current docs before writing code (none of this was in training-data knowledge — Copilot CLI and JetBrains support are 2025-era additions) and re-ran the throwaway-HOME install test to confirm all new config files come out right and still merge without clobbering. Deliberately did NOT add Eclipse/Xcode (also Copilot-MCP-capable per GitHub's changelog) — user's ask was specifically "cli, jetbrains, etc" and those two have far less certain config-path documentation; add only if asked. **Gotcha this surfaced:** committing a version bump doesn't mean it's published — always check `npm view <pkg> versions` before assuming a locally-committed version number is still available; this batch had to move from 1.7.0 to 1.8.0 because 1.7.0 got published mid-conversation, between the two rounds of work.
